...
// CORRIGIDO: Loggi agora é detectada corretamente
// =============================================
export function detectCarrier(trackingNumber: string): { name: string; code: number } {
  ...
  // IMPORTANTE: Código 190271 = Loggi Express BR
  // Exemplo: GBEFUWCT, GNTUMZCG
  // =============================================
  if (/^[A-Z0-9]{8}$/.test(tracking) && !tracking.endsWith('BR')) {
    // Verificar se parece código Loggi (geralmente começa com G ou tem padrão específico)
    return { name: 'Loggi', code: 190271 }; // Loggi Express BR
  }
  ...
  if (/^[A-Z]{2}\d{9}BR$/.test(tracking)) {
    return { name: 'Correios', code: 100023 }; // Correios Brasil
  }
...
export function formatPhoneForWhatsApp(phone: string | null): string | null {
  ...
  // Retorna apenas se tiver tamanho válido (13 dígitos: 55 + DDD + número)
